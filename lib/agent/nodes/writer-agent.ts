...
export async function writerNode(state: BlogState): Promise<Partial<BlogState>> {
  ...
  if (photoGrouping && photoGrouping.groups.length > 0) {
    ...
    return writeFromGroups(photoGrouping, styleProfile, photoAnalysis);
  }

  // 기존 방식 (이미지별 섹션)
  console.log('[Writer Agent] 이미지별 섹션 생성 모드');
  return writeFromImages(photoAnalysis, styleProfile, images);
}
...
**그룹 ${idx + 1}: ${g.title}** (${g.imageCount}장)
- 메인 아이템: ${g.mainItem}
- 카테고리: ${g.category}
- 설명: ${g.description}
...
[그룹 1: ${groups[0]?.title || '첫 번째'}에 대한 내용]

[그룹 2: ${groups[1]?.title || '두 번째'}에 대한 내용]
